globals
[
  ;; Track the count of poeple in each state
  susceptible-count
  dead-count
  infected-count
  immune-count

  ;; Track the number of infections per person and
  ;; the number of interactions per person
  average-infections
  near-count

  ;; Define rates parameters
  elderly-mortality-rate
  youth-mortality-rate
  elderly-probability
  initially-infected-probability
...
[
  ...
  immune?             ;; If true, the person has lived through an infection and become immune
  dead?               ;; If true, the person is removed from the simulation
  susceptible?        ;; If true, the person is susceptible to becoming infected
  virgin?             ;; Tracks whether the person has had the virus or not
  infection-length    ;; Tracks how long the person has been infected
  infectious-time     ;; The period of time an inividual will be infected, normally distributed around average-infectious-time
]

;; Define two age groups based on whether the person is over 65 years or not
breed [ elderly old ]
breed [ youth young ]
...
to assign-parameters
  set elderly-mortality-rate 10
  set youth-mortality-rate 1
  set elderly-probability 15.6
  set initially-infected-probability 5
  set average-infectious-time 14
end
...
to setup-people
  ...
  [
    ;; Initialise people in random location, susceptible
    ...
    ;; Set the infectious time for each agent to fall on a
    ;; normal distribution around average recovery time
    set infectious-time random-normal average-infectious-time average-infectious-time / 4

    ;; make sure it lies between 0 and 2x average-recovery-time
    if infectious-time > average-infectious-time * 2 [
      set infectious-time average-infectious-time * 2
    ]
    if infectious-time < 0 [ set infectious-time 0 ]

    ;; Each individual has a 5% chance of starting out infected.
    ;; To mimic true KM conditions use "ask one-of turtles" instead.
    if (random-float 100 < initially-infected-probability)
    [
      set infected? true
      set susceptible? false
      ;; If they start infected, they have been infected for a random amount of time
      set infection-length random infectious-time
    ]

    ;; Each individual is defined as either "old" or "young",
    ifelse random-float 100 < elderly-probability
    [ set breed elderly ]
    [ set breed youth ]
    assign-color
  ]
...
to go
  ...
  ;; Infected people infect others at each tick
  ;; Infected people live or die if passed their infectious period
...
;; Function used to calculate the mean number of interactions per person per day
to count-nearby
  set near-count (near-count + count turtles-on neighbors)
end
...
to infect  ;; turtle procedure
   let nearby-susceptible (turtles-on neighbors)
    with [susceptible? and not immune?]

     if nearby-susceptible != nobody
     [ ask nearby-susceptible
       ;; If true, infect nearby people
       [ if random-float 100 < infection-chance
         [ set infected? true
           ...
           set susceptible? false
           set virgin? false
           set average-infections (average-infections + 1)
         ]
       ]
     ]
end

;; Function which determines whether one lives or dies at the end of their infectious period
to live-or-die
  set infection-length infection-length + 1
  ;; If people have been infected for more than the recovery-time
  ;; then they either recover or die
  if infection-length > infectious-time
  [
    ;; Apply death probablities according to whether old or young
    ifelse breed = elderly
    [
      ifelse random-float 100 < elderly-mortality-rate
      [ dead ]
      [ immune-or-susceptible ]
    ]
    [
      ifelse random-float 100 < youth-mortality-rate
      [ dead ]
      [ immune-or-susceptible ]
    ]
  ]
...
;; Applies change of state to immune or susceptible
to immune-or-susceptible
  set infected? false
  ;; If true, person transitions to immune state, otherwise they transition to the susceptible state
  ifelse random-float 100 < immunity-chance
  [ set immune? true
...
;; Function which changes a person to the dead state; they are removed from the simulation
to dead
...
"" ""
PENS
"deaths" 1.0 0 -16777216 true "" "plot dead-count"
...
227
396
Mean interactions per day per person
(near-count / initial-people) / ticks
2
1
11
...
227
448
% of population infected at least once
(initial-people - (count turtles with [virgin?])) / initial-people * 100
2
1
11
...
average-infections / initial-people
17
1
11
